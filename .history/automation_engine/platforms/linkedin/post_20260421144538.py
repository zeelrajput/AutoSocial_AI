...
def post_to_linkedin(driver, post):
    try:
        ...
        def find_post_button_inside_active_dialog(driver):
            try:
                btn = driver.execute_script("""
                    const visible = (el) => {
                        if (!el) return false;
                    ...
                    buttons.sort((a, b) => {
                        ...
                        if (br.top !== ar.top) return br.top - ar.top;   // lower button first
                        return br.left - ar.left; // right-most first
                    });

                    return buttons[0];
                """)
                if btn:
                    print("✅ Real Post button found inside active dialog")
                    return btn
            except Exception as e:
                print("⚠️ find_post_button_inside_active_dialog failed:", e)

            return None

        def wait_until_real_post_button_enabled(driver, timeout=10):
            end_time = time.time() + timeout

            while time.time() < end_time:
                try:
                    btn = find_post_button_inside_active_dialog(driver)
                    if btn:
        ...
        post_button = wait_until_real_post_button_enabled(driver, timeout=10)
        if not post_button:
        ...
        before_body = ""
        try:
            before_body = driver.execute_script("return (document.body.innerText || '').toLowerCase();")
        except Exception:
            pass

...
def click_create_post_from_activity(driver, activity_section):
    try:
        btn = activity_section.find_element(
            By.XPATH,
            ".//a[contains(@href, '/preload/sharebox')]"
        )

        print("✅ Create post link found")

        # 🚀 IMPORTANT: force navigation instead of click
        driver.get("https://www.linkedin.com/feed/?shareActive=true")
        time.sleep(5)
        print("✅ Forced composer open via URL")

        return True

    except Exception as e:
        print("❌ Create post failed:", str(e))
        return False
        ...
        for el in elems:
            try:
                ...
                if el.is_displayed() and el.size['height'] > 50:
                    print("✅ Real LinkedIn textbox found")
                    return el
            except:
                continue

    except Exception as e:
        print("⚠️ textbox search error:", str(e))
...
def find_post_button(driver, textbox=None, timeout=12):
    end_time = time.time() + timeout
    ...
    while time.time() < end_time:
        try:
            result = driver.execute_script("""
                ...
                for (const c of extraContainers) {
                    if (visible(c)) containers.push(c);
                }
                    ...
                    for (const btn of buttons) {
                        ...
                        if (
                            ...
                            (
                                t === 'post' ||
                                a === 'post' ||
                                (t.includes('post') && !t.includes('menu'))
                            )
                        ){
                            return { found: true, button: btn, debug: debug };
                        }
                    }
                }
            ...
            print("⚠️ find_post_button JS scan failed:", str(e))
            time.sleep(1)
            continue

        if result and result.get("debug") is not None:
            print("🧾 Dialog buttons debug:", result["debug"][:20])

        if result and result.get("found"):
            btn = result["button"]
            print("✅ Post button found from textbox-parent scan")
...
# wake up LinkedIn so Post button gets enabled
        try:
            ...
            print("✅ Triggered beforeinput/input/change/blur events")

        except Exception as e:
            print("⚠️ Wake-up script failed:", str(e))

        # extra real-key fallback
        try:
            textbox.send_keys(" ")
            textbox.send_keys(Keys.BACKSPACE)
            print("✅ Sent SPACE + BACKSPACE to activate LinkedIn composer")
        except Exception as e:
            print("⚠️ SPACE/BACKSPACE fallback failed:", str(e))

        short_pause(3)
        medium_pause()

        
        post_button = find_post_button(driver, textbox=textbox, timeout=12)
        ...
        if not clicked:
            try:
                ...
                """, post_button)
                clicked = True
                print("✅ Post button clicked using JS mouse events")
            except Exception as e:

        ...
        try:
            ...
            for el in elements:
                try:
                    ...
                    # fallback: editable-looking paragraph/div
                    if tag_name in ["div", "p"] and ("write" in text or "share" in text):
...
def wait_for_linkedin_composer(driver, timeout=20):
    ...
    while time.time() < end_time:
        ...
        # 🔥 2. NEW: Global editor search (INLINE UI FIX)
        try:
            global_editors = driver.find_elements(By.CSS_SELECTOR, "div[contenteditable='true'], textarea")
                ...
                try:
                    if el.is_displayed():
                        ...
                        if (
                            "ql-editor" in cls
                            or role == "textbox"
                            or el.get_attribute("contenteditable") == "true"
                        ):

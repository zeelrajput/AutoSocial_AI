...
def get_outer_html(driver, element, limit=800):
    try:
...
def wait_for_composer_surface(driver, timeout=12):
        ...
        try:
            found = driver.execute_script("""
                ...
                const dialogs = [
                    ...
                    ...document.querySelectorAll("[data-test-modal]"),
                    ...document.querySelectorAll(".share-box-feed-entry__closed"),
            ...
            print("⚠️ Composer check JS failed:", e)
                ...
                try:
                    ...
                    try:
                        parent_html = driver.execute_script("""
                            const el = arguments[0];
                            const p = el.closest("div[role='dialog'], div.artdeco-modal, form, section");
                            return p ? p.outerHTML.slice(0, 1500).toLowerCase() : '';
                        """, el)
...
def find_post_button(driver):
    # Try normal xpath search first
    btn = find_first_visible_by_xpaths(driver, POST_BUTTON_XPATHS, label="Post button")
    if btn:
        return btn

    # JS fallback: find visible enabled button containing "Post"
    try:
        btn = driver.execute_script("""
            ...
            const buttons = [...document.querySelectorAll("button")].filter(b => {
                const text = (b.innerText || b.textContent || b.ariaLabel || '').trim().toLowerCase();
                const disabled = b.disabled || b.getAttribute("aria-disabled") === "true";
                return visible(b) && !disabled && text.includes("post");
            });
        ...
        print("⚠️ JS fallback for post button failed:", e)
...
def post_to_linkedin(driver, post):
    try:
        ...
        opened = wait_for_composer_surface(driver, timeout=12)
        if not opened:
            print("⚠️ Composer detection failed, trying textbox search anyway...")

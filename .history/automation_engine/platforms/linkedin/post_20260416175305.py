            ...
            for el in elements:
                try:
                    if el.is_displayed() and el.is_enabled():
                        print("✅ REAL Start Post found using:", xpath)
                        return el
                except:
                    continue
        except:
...
def get_real_editor(driver, timeout=12):
    ...
    while time.time() < end_time:
        ...
        # Step 1: find dialog first (VERY IMPORTANT)
        ...
        for dialog in dialogs:
            try:
                ...
                # Step 2: find editor INSIDE dialog only
        ...
        # fallback: click placeholder to activate editor
        try:
...
def type_in_editor(driver, textbox, text):
    ...
    # 3. ActionChains on focused editor
    try:
...
def post_to_linkedin(driver, post):
    try:
        ...
        # Do NOT fail here if popup detection misses.
        # Try to activate the composer manually first.
        try:
            placeholders = driver.find_elements(
                By.XPATH,
                "//*[contains(normalize-space(), 'What do you want to talk about?')]"
        ...
        if not clicked:
            ttry:
                ...
                # Always try JS click first (LinkedIn needs it)
                driver.execute_script("arguments[0].click();", start_post_button)
                print("Start post clicked using JavaScript")

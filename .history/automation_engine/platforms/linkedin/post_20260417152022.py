...
def post_to_linkedin(driver, post):
    try:
        ...
        # Instead of searching for placeholder text, we click the top-center of the modal
        try:
            dialog = driver.find_element(By.XPATH, "//div[@role='dialog'] | //div[contains(@class, 'share-box')]")
            # Click 80 pixels down from the top-center of the dialog (the editor sweet spot)
            ActionChains(driver).move_to_element_with_offset(dialog, 0, 80).click().perform()
            print("✅ Dialog clicked at offset to force focus")
            time.sleep(2)
        except Exception as e:
            print(f"⚠️ Offset click failed: {str(e)}")
        ...
        # --- STEP 3: TYPE ---
        print("✅ Editor detected. Entering text...")
        # We use insertText because it triggers the 'input' event LinkedIn needs to enable the Post button
        driver.execute_script("""
            arguments[0].focus();
        ...
        try:
            ...
            # Final attempt at Post button if standard find fails

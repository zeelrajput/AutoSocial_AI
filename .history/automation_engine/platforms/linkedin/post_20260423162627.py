...
def wait_for_modal(driver, timeout=12):
    ...
    while time.time() < end_time:
        try:
            ...
            if visible_dialogs:
                print("✅ LinkedIn modal composer detected")
                return True
        except Exception as e:
            print(f"⚠️ wait_for_modal error: {e}")
        time.sleep(1)
    return False


def force_open_linkedin_modal(driver, start_btn, timeout=12):
    print("🔄 Forcing LinkedIn modal composer open...")
    ...
    # Try normal robust click first
    click_element_robustly(driver, start_btn, "Start Post")

    if wait_for_modal(driver, timeout=3):
        return True
    ...
    # Try ENTER on start button
    try:
        start_btn.send_keys(Keys.ENTER)
        print("✅ ENTER sent on Start Post")
        time.sleep(2)
    except Exception as e:
        print(f"⚠️ ENTER on Start Post failed: {e}")
...
def wait_for_composer_open(driver, timeout=10):
    # Kept for compatibility, but modal is preferred
    end_time = time.time() + timeout
    while time.time() < end_time:
        try:
    ...
    while time.time() < end_time:
        try:
            result = driver.execute_script("""
                ...
                if (dialogs.length === 0 && editors.length === 0) {
...
def find_modal_post_button(driver):
    try:
        buttons = driver.find_elements(
            By.XPATH,
            "//div[@role='dialog']//*[self::button or self::div[@role='button']]"
        )
        ...
        for btn in buttons:
            try:
                ...
                text = (
                    ...
                    (btn.get_attribute("aria-label") or "").strip().lower() + " " +
                    (btn.get_attribute("title") or "").strip().lower()
                ).strip()
                ...
                if text == "post" or text.startswith("post "):
                    print("✅ Real modal Post button found")
                    print("🧾 Post button text:", text)
                    return btn
            except Exception:
                continue

    except Exception as e:
        print(f"⚠️ find_modal_post_button error: {e}")
...
def click_final_post_button(driver, textbox=None, retries=8):
    ...
    for i in range(retries):
        try:
            post_btn = find_modal_post_button(driver)
            if post_btn:
                if click_element_robustly(driver, post_btn, "Final Post"):
                    time.sleep(5)
...
def post_to_linkedin(driver, post):
    ...
    try:
        ...
        if not force_open_linkedin_modal(driver, start_btn, timeout=12):
            return {"success": False, "message": "LinkedIn modal did not open"}

        textbox = find_linkedin_textbox(driver, timeout=20)
        if not textbox:
            return {"success": False, "message": "Textbox not found in modal"}

...
def click_create_post_from_activity(driver, activity_section):
    ...
    # ✅ FIX: Strip href to prevent link navigation and let JS modal handler fire
    try:
...
def try_global_start_post_button(driver):
    """
    Clicks the 'Start a post' button from the feed/home page top bar
    as a fallback when the activity section link fails.
    """
    try:
        print("🔄 Navigating to feed for Start a post fallback...")
        driver.get("https://www.linkedin.com/feed/")
        time.sleep(4)

        btn = find_first_visible_by_xpaths(driver, [
        ...
        ], label="Start a post button")

        if btn:
            robust_click(driver, btn, label="Start a post button")
            short_pause(2)
            return wait_for_composer_surface(driver, timeout=8)

    except Exception as e:
        print(f"⚠️ Global start post fallback failed: {e}")
...
def dismiss_draft_dialog(driver, timeout=5):
    """
    Detects and dismisses the 'Save this post as a draft?' dialog
    by clicking 'Discard' to close it and return to the composer.
    """
    end_time = time.time() + timeout
    ...
    while time.time() < end_time:
        try:
            dismissed = driver.execute_script("""
                ...
                if (!bodyText.includes('save this post as a draft')) return false;

                const buttons = Array.from(document.querySelectorAll('button, span[role="button"]'));
                const discard = buttons.find(b =>
                    (b.innerText || b.textContent || '').trim().toLowerCase() === 'discard'
                );
                if (discard) {
                    discard.click();
                    return true;
            ...
            if dismissed:
                print("✅ Draft dialog dismissed (Discard clicked)")
                short_pause(1.5)
                return True
...
def post_to_linkedin(driver, post):
    try:
        ...
        # ✅ FIX: If composer didn't open from activity section, fall back to feed
        opened = wait_for_composer_surface(driver, timeout=10)
        if not opened:
            print("⚠️ Composer not detected via activity click, trying feed fallback...")
            opened = try_global_start_post_button(driver)

        if not opened:
            print("⚠️ All composer open attempts failed, proceeding anyway...")
        ...
        # ✅ FIX: Dismiss draft dialog that may have appeared during/after typing
        dismiss_draft_dialog(driver, timeout=5)

        post_button = None
        for _ in range(3):
            # ✅ FIX: Guard each retry in case dialog reappears with a delay

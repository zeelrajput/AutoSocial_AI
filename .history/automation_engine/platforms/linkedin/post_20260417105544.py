...
def find_linkedin_textbox(driver):
    xpath_candidates = [
        ...
        "//div[@role='dialog']//*[contains(text(),'What do you want to talk about?')]",
...
def wait_for_linkedin_composer(driver, timeout=15):
    ...
    while time.time() < end_time:
        ...
        try:
            editable = driver.find_elements(By.XPATH, "//*[@contenteditable='true']")
            print("Editable elements found:", len(editable))
        except Exception:
            pass

        try:
            placeholders = driver.find_elements(By.XPATH, "//*[@data-placeholder or @aria-placeholder]")
            print("Placeholder elements found:", len(placeholders))
        except Exception:
            pass
...
def type_into_linkedin_editor(driver, textbox, caption):
    try:
        ...
        # ActionChains style fallback via JS event dispatch
        try:
            driver.execute_script("""
                ...
                el.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, key: 'H' }));
                el.dispatchEvent(new KeyboardEvent('keyup', { bubbles: true, key: 'H' }));
            """, textbox, caption)

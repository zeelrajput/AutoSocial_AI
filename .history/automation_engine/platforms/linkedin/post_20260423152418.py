    ...
    for i in range(retries):
        try:
            result = driver.execute_script("""
                ...
                // 1. First try visible dialog
                const dialogs = Array.from(document.querySelectorAll("div[role='dialog']"))
                    .filter(isVisible);

                if (dialogs.length) {
                    const dialogRoot = dialogs[dialogs.length - 1];
                    const btn = findPostButton(dialogRoot);
                    if (btn) {
                        btn.click();
                        return {
                            clicked: true,
                ...
                // 2. Try near active editor / inline composer
                ...
                for (const editor of editors) {
                    ...
                    for (let depth = 0; depth < 6 && node; depth++) {
                        const btn = findPostButton(node);
                        if (btn) {
                            btn.click();
                            return {
                                clicked: true,
                                mode: "inline_ancestor",
                                html: btn.outerHTML.slice(0, 500)
                            };
                        }
                ...
                // 3. Global fallback
                const globalBtn = findPostButton(document);
                if (globalBtn) {
                    globalBtn.click();
                    return {
                        clicked: true,
                        mode: "global",
                        html: globalBtn.outerHTML.slice(0, 500)
                    };
                }

                return {
                    clicked: false,
                    reason: dialogs.length ? "post_not_found_in_dialog_or_inline" : "no_dialog_and_no_inline_post"
                };
            """)

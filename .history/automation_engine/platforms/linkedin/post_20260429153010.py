...
from automation_engine.common.tab_manager import o
...
def upload_files_with_windows_dialog(media_files, timeout=15):
    ...
    print("🪟 Waiting for Windows file dialog...")

    end_time = time.time() + timeout

    while time.time() < end_time:
        try:
            dialog = Desktop(backend="uia").window(title_re=".*Open.*|.*Upload.*|.*Choose.*")
            ...
            if dialog.exists(timeout=1):
                ...
                open_btn = dialog.child_window(title_re=".*Open.*|.*Upload.*", control_type="Button")
                open_btn.click()

                print("✅ Files selected from Windows dialog:", media_files)
                return True

        except Exception as e:
            print("⚠️ Waiting file dialog:", str(e))

        time.sleep(1)

    print("❌ Windows file dialog not found")
    return False
...
def upload_media_if_present(driver, textbox, post):
    ...
    if not file_input:
        ...
        if photo_btn:
            try:
                photo_btn.click()
                print("✅ Photo Button clicked using normal click")
            except Exception:
                driver.execute_script("arguments[0].click();", photo_btn)
                print("✅ Photo Button clicked using JS fallback")

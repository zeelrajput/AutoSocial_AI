...
def find_post_button(driver):
    # 1. XPath based search
    btn = find_first_visible_by_xpaths(
        driver,
        POST_BUTTON_XPATHS,
        label="Post button",
    ...
    # 2. JS fallback: visible enabled button with text/label Post
    try:
        btn = driver.execute_script("""
            ...
            const buttons = [...document.querySelectorAll("button")].filter(b => {
                ...
                return visible(b) && enabled(b) && text.includes('post');
            });

        ...
        try:
            ...
            for dialog in dialogs:
                try:
                    ...
                    editors = dialog.find_elements(
                        By.CSS_SELECTOR,
                        "div.ql-editor[contenteditable='true'], div[role='textbox'][contenteditable='true'], div[contenteditable='true']"
                    )
                    post_buttons = dialog.find_elements(
                        By.XPATH,
                        ".//button[@aria-label='Post' or contains(@aria-label,'Post') or .//span[normalize-space()='Post']]"
                    )

                    if any(e.is_displayed() for e in editors) or any(b.is_displayed() for b in post_buttons):
                        print("Composer surface detected inside dialog")
                        return dialog
                except Exception:
                    continue
        except Exception:
...
def post_to_linkedin(driver, post):
    try:
        ...
        dialog = wait_for_composer_surface(driver, timeout=12)
if not dialog:
    screenshot = save_screenshot(driver, prefix="linkedin_composer_not_opened")
    return {
        "success": False,
        "message": f"LinkedIn composer not opened. Screenshot: {screenshot}"
    }

        ...
        try:
            result = driver.execute_script("""
                ...
                function isRealSubmitButton(el) {
                    ...
                    // reject composer opener buttons
                    const blocked = [
                        'start a post',
                        'create a post',
                        'post to anyone',
                ...
                if (dialogs.length) {
                    ...
                    const btn = findSubmitInRoot(dialogRoot);
                    if (btn) {
                        btn.click();
                        return {
                            clicked: true,
                ...
                // 2. search around visible editors only
                const editors = Array.from(document.querySelectorAll(
                    ".ql-editor[contenteditable='true'], div[role='textbox'][contenteditable='true'], div[contenteditable='true']"
                )).filter(isVisible);
                ...
                return {
                    clicked: false,
                    reason: dialogs.length ? "submit_not_found_near_composer" : "no_dialog_and_no_submit_near_editor"
                };
            """)

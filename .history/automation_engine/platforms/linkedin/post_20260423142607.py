    ...
    for i in range(retries):
        try:
            result = driver.execute_script("""
                ...
                const roots = [];
                if (dialogs.length) {
                    roots.push(dialogs[dialogs.length - 1]);
                }
                roots.push(document);

                for (const root of roots) {
                    const candidates = Array.from(
                        root.querySelectorAll("button, div[role='button'], [aria-label]")
                    );
                ...
                const all = Array.from(document.querySelectorAll("button, div[role='button'], [aria-label]"));
                const debug = all
                    .filter(isVisible)
                    .slice(0, 40)
                    .map((el, idx) => {
            ...
            print(f"⚠️ Retry {i+1}: Post button not clicked")
            if result and result.get("debug"):
                print("🧾 Visible button candidates:", result.get("debug"))
...
def post_to_linkedin(driver, post):
    ...
    try:
        ...
        if not is_composer_open(driver):
            return {"success": False, "message": "Composer closed after image upload"}
            
        if not click_final_post_button(driver):

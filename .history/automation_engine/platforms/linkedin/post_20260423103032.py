    ...
    while time.time() < end_time:
        try:
            opened = driver.execute_script("""
                ...
                if (
                    pageText.includes('post to anyone') ||
                    pageText.includes('what do you want to talk about') ||
                    pageText.includes('create a post')
                ) {
                    return true;
                }
    ...
    for idx, el in enumerate(unique_candidates):
        for action_name in [
            ...
            try:
                ...
                elif action_name == "enter_key":
                    el.send_keys(Keys.ENTER)

                elif action_name == "space_key":
                    el.send_keys(Keys.SPACE)

                elif action_name == "js_mouse_events":
...
def post_to_linkedin(driver, post):
    ...
    try:
        ...
        if not click_start_post(driver, start_btn):
            try:
                preview = driver.execute_script("return (document.body.innerText || '').slice(0, 3000);")
                print("🧾 LinkedIn page preview after start-post attempts:", preview)
            except Exception:
                pass
        ...
        if composer_open:
            ...
            try:
                preview = driver.execute_script("return (document.body.innerText || '').slice(0, 3000);")
                print("🧾 LinkedIn page preview when composer not detected:", preview)
            except Exception:
                pass

...
# ✅ NEW: dismiss draft dialog if it appeared
...
# ✅ NEW: find post button using JS directly — most reliable method
def find_post_button_js(driver):
    try:
...
def find_post_button(driver):
    # First try JS method (fastest and most reliable)
    btn = find_post_button_js(driver)
    if btn:
        return btn
...
def post_to_linkedin(driver, post):
    try:
        ...
        # ✅ STEP 6: Dismiss draft dialog + find and click Post button
        ...
        for attempt in range(4):
            ...
            if post_button:
                break
            print(f"🔄 Post button not found, retrying... ({attempt + 1}/4)")
            short_pause(1.5)

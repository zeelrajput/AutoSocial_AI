...
def click_final_post_button(driver, textbox, retries=5):
    time.sleep(3)
    print("🔄 Finding REAL Post button...")
    ...
    for i in range(retries):
        try:
            ...
            if post_btn:
                print(f"✅ Post button found (retry {i+1})")

                # FORCE CLICK (important)
                driver.execute_script("arguments[0].click();", post_btn)
                print("✅ Post button clicked using JS")
...
try:
    driver.get("https://www.linkedin.com/in/zeel-rajput-25010136b/recent-activity/all/")
    time.sleep(6)

    page_text = driver.execute_script("return document.body.innerText || '';")

    if caption[:25] in page_text:
        return {"success": True, "message": "LinkedIn post successful and verified on activity page"}

    driver.get("https://www.linkedin.com/feed/")
    time.sleep(5)
    feed_text = driver.execute_script("return document.body.innerText || '';")

    if caption[:25] in feed_text:
        return {"success": True, "message": "LinkedIn post successful and verified on feed"}

    return {"success": False, "message": "Post button clicked, but post not verified"}
except Exception as e:
    return {"success": False, "message": f"Post clicked, but verification failed: {str(e)}"}
    except Exception as e:

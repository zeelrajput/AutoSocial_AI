...
def get_real_editor(driver, timeout=15):
    ...
    while time.time() < end_time:
        # 1. Broadest possible search for the dialog container
        containers = driver.find_elements(By.XPATH, "//div[contains(@class, 'share-box')] | //div[@role='dialog'] | //div[contains(@class, 'artdeco-modal')]")
        
        for container in containers:
            try:
                # 2. Updated Editor Selectors
                editor_selectors = [
                    ".//div[@role='textbox']",
                    ".//div[contains(@class, 'ql-editor')]",
                    ".//div[@contenteditable='true']",
                    ".//div[contains(@aria-label, 'editor')]",
                    ".//p" # Fallback to the paragraph tag inside the editor
                ]

                for selector in editor_selectors:
                    editors = container.find_elements(By.XPATH, selector)
        ...
        # If no editor found in containers, try searching globally (last resort)
        global_editors = driver.find_elements(By.XPATH, "//div[@role='textbox' and @contenteditable='true']")
        if global_editors:
            return global_editors[0]
...
def post_to_linkedin(driver, post):
    try:
        ...
        try:
            ...
            # Try JS Click First - it's often more reliable for LinkedIn modals
            driver.execute_script("arguments[0].click();", start_post_button)
            print("✅ Start post clicked (JS)")
            
            # Extra safeguard: if the click didn't happen, try ActionChains
            time.sleep(1) 
        except Exception as e:
            # ... keep error handling ...

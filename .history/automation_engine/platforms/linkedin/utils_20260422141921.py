...
def find_textbox(driver, timeout=20):
    end_time = time.time() + timeout
    ...
    while time.time() < end_time:
            ...
            try:
                ...
                for el in elements:
                    try:
                        ...
                        if role == "textbox" or ce == "true" or "ql-editor" in cls:
                            print(f"✅ Textbox found using XPath: {xpath}")
                            try:
        ...
        # 2. JS fallback: modal OR inline composer
        try:
            el = driver.execute_script("""
                function isVisible(el) {
                    if (!el) return false;

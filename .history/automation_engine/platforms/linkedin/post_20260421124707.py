...
def post_to_linkedin(driver, post):
    try:
        ...
        # 1. Navigate and find activity section
        ...
        if not activity_section:
            ...
            return {"success": False, "message": f"Activity section not found. Screenshot: {screenshot}"}
        ...
        if not clicked:
            ...
            return {"success": False, "message": f"Create a post button click failed. Screenshot: {screenshot}"}

        short_pause(2)
        wait_for_composer_surface(driver, timeout=10)

        # 3. Find and type into the textbox
        ...
        if not typed:
            ...
            return {"success": False, "message": f"Typing failed. Screenshot: {screenshot}"}

        # --- STEP 4: HANDLE BLOCKING MODALS (The "Save Draft" Fix) ---
        short_pause(2)
        try:
            # Check if the "Save this post as a draft?" modal is blocking the Post button
            draft_modal_discard = driver.find_elements(By.XPATH, "//button[contains(., 'Discard')]")
            if draft_modal_discard:
                for btn in draft_modal_discard:
                    if is_visible(driver, btn):
                        print("⚠️ 'Save draft' modal detected. Clicking Discard to clear view.")
                        robust_click(driver, btn, label="Discard button")
                        short_pause(1.5)
                        break
        except Exception as e:
            print(f"Draft modal check skipped: {e}")

        # --- STEP 5: FIND AND CLICK POST BUTTON ---
        post_button = None
        # Try a few times in case the UI is still animating after the modal clear
        for attempt in range(4):
            post_button = find_post_button(driver)
            if post_button:
                # Force scroll to ensure the button is in the clickable viewport
                driver.execute_script("arguments[0].scrollIntoView({block:'center'});", post_button)
                if is_visible(driver, post_button):
                    break
            print(f"🔄 Searching for Post button... attempt {attempt + 1}")
            short_pause(1.5)
        ...
        if not clicked:
            ...
            return {"success": False, "message": "Post button click failed."}
        ...
        return {"success": False, "message": "Post click happened, but success not confirmed."}

...
def get_activated_editor(driver):
    ...
    if (placeholder) {
        // 2. Fire a full sequence of events to wake up React
        placeholder.focus();
        triggerMouseEvent(placeholder, 'mousedown');
        triggerMouseEvent(placeholder, 'click');
        triggerMouseEvent(placeholder, 'mouseup');
    }
...
# =========================
# MAIN FUNCTION (Refined Step 2 & 3)
# =========================
def post_to_linkedin(driver, post):
    try:
        ...
        # --- STEP 2: ACTIVATE EDITOR ---
        textbox = get_activated_editor(driver)
        
        # Fallback: If the JS event didn't work, try a physical click via ActionChains
        if not textbox:
            print("...JS activation failed, trying physical ActionChains click...")
            try:
                # Find the dialog box and click slightly below the top to hit the placeholder area
                dialog = driver.find_element(By.XPATH, "//div[@role='dialog']")
                ActionChains(driver).move_to_element_with_offset(dialog, 0, 50).click().perform()
                time.sleep(2)
                textbox = get_activated_editor(driver)
            except:
                pass
        ...
        if not textbox:
            screenshot = save_screenshot(driver, prefix="activation_failure")
            return {"success": False, "message": "Could not activate editor."}
        ...
        # --- STEP 4: POST ---
        time.sleep(3) # Give LinkedIn a second to enable the button
        post_btn_xpath = "//button[contains(@class, 'share-actions__primary-action')] | //button[contains(., 'Post')]"

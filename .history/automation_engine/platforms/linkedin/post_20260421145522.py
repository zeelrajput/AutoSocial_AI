...
def click_real_post_button(driver, post_button):
    try:
        ...
        print("✅ Real Post button clicked using ActionChains")
        return True
    except Exception as e:
        print("⚠️ ActionChains click failed:", e)

    try:
        post_button.click()
        print("✅ Real Post button clicked using selenium click")
        return True
    except Exception as e:
        print("⚠️ Selenium click failed:", e)

    try:
        driver.execute_script("arguments[0].click();", post_button)
        print("✅ Real Post button clicked using JS click")
        return True
    except Exception as e:
        print("⚠️ JS click failed:", e)
...
def find_real_post_button(driver):
    try:
        btn = driver.execute_script("""
            ...
            let buttons = [...dialog.querySelectorAll("button")].filter(b => {
                const txt = textOf(b);
            ...
            if (!buttons.length) {
                buttons = [...dialog.querySelectorAll("*")].filter(el => {
                    if (el.tagName !== 'BUTTON') return false;
                    const txt = textOf(el);
                    return visible(el) && enabled(el) && txt.includes('post');
                });
            }
            ...
            buttons.sort((a, b) => {
                ...
                if (br.bottom !== ar.bottom) return br.bottom - ar.bottom;
        ...
        """)
        if btn:
            print("✅ Real Post button found using dialog JS search")
            return btn
    except Exception as e:
        print("⚠️ find_real_post_button failed:", e)

    return None


def wait_until_real_post_button_enabled(driver, timeout=12):
    end_time = time.time() + timeout

    while time.time() < end_time:
        try:
            btn = find_real_post_button(driver)
            if btn and is_enabled(driver, btn):
                print("✅ Real Post button is enabled")
                print("Post button HTML:", get_outer_html(driver, btn))
                return btn
        except Exception as e:
            print("⚠️ Waiting for real Post button failed:", e)
    ...
    while time.time() < end_time:
        try:
            confirmed = driver.execute_script("""
                ...
                if (bodyText.includes('save this post as a draft')) {
                    return false;
                }
...
def debug_visible_dialog_buttons(driver):
    try:
        debug_buttons = driver.execute_script("""
            const visible = (el) => {
                if (!el) return false;
                const r = el.getBoundingClientRect();
                const s = window.getComputedStyle(el);
            ...
            const dialog = dialogs[dialogs.length - 1];
            return [...dialog.querySelectorAll("button")].filter(visible).map(b => ({
                text: ((b.innerText || '') + ' ' + (b.getAttribute('aria-label') || '')).trim(),
                html: b.outerHTML.slice(0, 300)
            }));
        """)
        print("🔍 Visible buttons inside dialog:", debug_buttons)
    except Exception as e:
        print("⚠️ Dialog button debug failed:", e)
...
def post_to_linkedin(driver, post):
    try:
        ...
        post_button = wait_until_real_post_button_enabled(driver, timeout=12)
        if not post_button:
            debug_visible_dialog_buttons(driver)
            screenshot = save_screenshot(driver, prefix="linkedin_post_button_not_found")
            return {
                "success": False,
                "message": f"LinkedIn real Post button not found. Screenshot: {screenshot}"
            }

        clicked = click_real_post_button(driver, post_button)
        if not clicked:
        ...
        try:
            after_body = driver.execute_script("return (document.body.innerText || '').toLowerCase();")
        except Exception:
            after_body = ""

        if "save this post as a draft" in after_body:
            screenshot = save_screenshot(driver, prefix="linkedin_wrong_button_clicked")
            return {
                "success": False,
                "message": f"Wrong button got clicked and draft popup opened. Screenshot: {screenshot}"
            }

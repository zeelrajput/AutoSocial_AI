...
def remove_linkedin_overlay(driver):
    try:
        ...
        print("⚠️ Overlay removal failed:", str(e))
        ...
        try:
            btn = driver.execute_script("""
                ...
                const candidates = Array.from(document.querySelectorAll(
                    "button, div[role='button'], [aria-label='Start a post'], [aria-label*='Start a post']"
                ));
            ...
            if btn:
                print("✅ Start post button found using JS finder")
                try:
                    print("Start post button HTML:", btn.get_attribute("outerHTML")[:500])
                except Exception:
                    pass
                return btn

        except Exception as e:
            print("⚠️ JS start post finder failed:", str(e))
    ...
    while time.time() < end_time:
        try:
            photo_btn = driver.execute_script("""
                ...
                const candidates = Array.from(root.querySelectorAll("button, div[role='button'], label, span"));
            ...
            if photo_btn:
                print("✅ Photo button found inside composer/dialog")
                try:
                    print("Photo button HTML:", photo_btn.get_attribute("outerHTML")[:500])
                except Exception:
                    pass
                return photo_btn
    ...
    while time.time() < end_time:
        try:
            file_input = driver.execute_script("""
                ...
                if (allInputs.length) return allInputs[0];
            ...
            if file_input:
                ...
                print("✅ LinkedIn file input found")
                try:
                    print("File input HTML:", file_input.get_attribute("outerHTML")[:500])
                except Exception:
                    pass
                return file_input
...
def upload_linkedin_image(driver, image_path, timeout=20):
    try:
        ...
        print("✅ Waiting after Photo click...")
        time.sleep(3)

        try:
            dialog_debug = driver.execute_script("""
                const dialogs = Array.from(document.querySelectorAll("div[role='dialog']"));
                return dialogs.map((d, i) => `#${i}: ` + (d.outerHTML || '').slice(0, 500)).join("\\n\\n");
            """)
            print("🧾 Dialog debug after photo click:", dialog_debug)
        except Exception as e:
            print("⚠️ dialog debug failed:", str(e))

        print("Finding file input...")
        file_input = find_file_input(driver, timeout=timeout)
        if not file_input:
            try:
                input_debug = driver.execute_script("""
                    return Array.from(document.querySelectorAll("input"))
                        .map((el, i) => `#${i}: type=${el.type}, accept=${el.getAttribute('accept') || ''}, name=${el.getAttribute('name') || ''}, aria=${el.getAttribute('aria-label') || ''}`)
                        .join("\\n");
                """)
                print("🧾 Available inputs:", input_debug)
            except Exception as e:

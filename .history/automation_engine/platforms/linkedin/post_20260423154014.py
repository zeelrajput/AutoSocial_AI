    ...
    for i in range(retries):
        try:
            result = driver.execute_script("""
                ...
                // 2. Inline composer case: find visible editor
                const editors = Array.from(document.querySelectorAll(
                    ".ql-editor[contenteditable='true'], div[role='textbox'][contenteditable='true'], div[contenteditable='true']"
                )).filter(isVisible);

                if (!editors.length) {
                    return {clicked: false, reason: "no_visible_editor"};
                }

                const editor = editors[editors.length - 1];

                // Walk up and search each larger container fully
                let node = editor;
                ...
                // 3. Use geometric search: buttons near editor
                ...
                for (const el of candidates) {
                    ...
                    // prefer buttons below or to the right of editor
                    const dx = Math.abs(r.left - er.left);
                    const dy = Math.abs(r.top - er.bottom);

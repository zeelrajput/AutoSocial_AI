...
# DEFAULT_INSTAGRAM_IMAGE = r"D:\blockchain_internshipe\Autosocial_ai\default_images\instagram_default1.jpg"
...
def post_to_instagram(driver, post):
    ...
    try:
        ...
        if hasattr(post, "media") and post.media:
            try:
                # Old Django flow: post.media is FileField
                if hasattr(post.media, "path"):
                    image_path = post.media.path

                # New agent flow: post.media is already string path
                else:
                    image_path = str(post.media).strip()
        ...
        if not wait_for_instagram_login(driver, timeout=180):
            ...
            return {"success": False, "message": "Login not completed | {screenshot}"}
        ...
        if not clicked:
            return {"success": False, "message": "Share button click failed"}

...
def find_facebook_post_url(driver, caption, timeout=60):
    ...
    while time.time() < end:
        ...
        for article in articles:
            try:
                ...
                if caption and caption[:40] not in text:
                    continue

                links = article.find_elements(
                    By.XPATH,
                    ".//a[contains(@href,'/posts/') or contains(@href,'story_fbid=') or contains(@href,'permalink.php')]"
                )
                ...
                for link in links:
                    ...
                    if "comment_id=" in href or "/reel/" in href or "/watch/" in href:
                        continue
...
def post_to_facebook(driver, post):
    try:
        ...
        post_url = find_facebook_post_url(driver, caption, timeout=60)

...
def find_facebook_post_url(driver, timeout=30):
    try:
        # Open profile
        ...
        post_links = driver.find_elements(
            By.XPATH,
            "//div[@role='article']//a[contains(@href,'/posts/') or contains(@href,'story_fbid=') or contains(@href,'permalink.php')]"
        )
        ...
        for link in post_links:
            ...
            # Skip reels/videos
            if "/reel/" in href or "/watch/" in href:
                continue

            clean_url = href.split("&__cft__")[0]
            clean_url = clean_url.split("?__cft__")[0]
